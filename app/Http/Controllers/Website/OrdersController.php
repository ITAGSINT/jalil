...
use App\Http\Resources\OrderUserResource2;
...
class OrdersController extends Controller{
    ...
    public function getAllOrders(Request $request)
    {
        ...
        $ord_by = 'desc';
        $order = Order::with(['currents', 'driverName', 'products', 'address', 'car','method'])
            ->where('customers_id', $request->user_id);


        if ($request->status==5) {
            $order->whereRelation('currents', 'status_id', 5);
        }
        else {
            $order->whereRelation('currents', 'status_id','!=', 5);
        }
        ...
        $order = OrderUserResource2::collection($order);
    ...
    public function add2(Request $request)
    {
       ...
       // return $date;
        ...
        try {
            ...
            DB::commit();
            $request->session()->forget(['car_id', 'product_id','qty','','discount','old_total','new_total','code','address_id','time','date','time2','date2','price']);
            return response()->json($response, 200);
    ...
    public function applyCoupon(Request $request)
    {
        ...
        //check if order total is in range of applaying the coupon
        // if ($order_total >= $code->minimum_amount) {
        //     //ok it is above minimum continue
        //     if ($order_total <= $code->maximum_amount || $code->maximum_amount == 0) {
        //         //  ok it is below maximum continue
        //     } else {
        //         //over max
        //         return 'your order total is over than maximum amount to applay coupon';
        //     }
        // } else {
        //     //below min
        //     return 'your order total is leas than minimum amount to applay coupon';
        // }
        if ($code->type->id == 1) {
    ...
    private function checkouOnlinePayment($order, $serviceName)
    {
        ...
        $charge = $user->checkoutCharge((int)($order->order_price * 100), $serviceName, sessionOptions: [
            ...
            'success_url' => route('website.orders.payment_status', ['status' => 'success','id'=>$order->id]),
            'cancel_url' => route('website.orders.payment_status', ['status' => 'cancel','id'=>$order->id]),
        ]);

        return $charge;
    }
    public function getOrderPaymentHistory(Request $request) {
        $validator = Validator::make($request->all(), [
            'id' => ['required', 'exists:payment_history,id'],
        ]);
        ...
        $history=PaymentHistory::with(['method','status'])->where('id',$request->id)->first();
    ...
    public function addOrderWithcredit(Request $request)
    {
        $date = Carbon::createFromFormat('m/d/Y', $request->date);
    ...
    public function payment_status (Request $request) {
        if($request->status=='success') {
            ...
            $pay_history=PaymentHistory::where('id',$transaction)->first();
            $pay_history->status_id=5;
            $pay_history->update(); 
            $firebaseToken = User::whereIn('role_id', [1, 3])->pluck('fcm_token')->all();
...
public static function status_cancel(Request $request)
{
   // $request['user_id'] = Auth::guard('sanctum')->user()->id;
    ...
    $validator = Validator::make(
        ...
        [
           // 'user_id' => ['required', 'exists:users,id'],
            'order_id' => ['required', Rule::exists('orders', 'id')->where('customers_id', Auth::id())],
        ],
        [
        ...
        $orders = new OrderUserResource($order); {
            ...
            $data = [
                ...
                "notification" => [
                    ...
                    "body" => 'infor drivers of the order',
                    "mutable_content" => true,
                    "sound" => "Tri-tone",
